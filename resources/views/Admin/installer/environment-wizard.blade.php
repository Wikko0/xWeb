@extends('Admin.installer.layouts.master')
    ...
    <div class="tabs tabs-full">
        ...
        <form method="post" action="{{ route('LaravelInstaller::environmentSaveWizard') }}" class="tabs-wrap">
            <div class="tab" id="tab1content">
                ...
                <div class="form-group {{ $errors->has('mu_version') ? ' has-error ' : '' }}">
                    <label for="mu_version">
                        {{ trans('installer_messages.environment.wizard.form.mu_version') }}
                    </label>
                    <select name="mu_version" id="mu_version">
                        <option value="s6_muemu" selected>{{ trans('installer_messages.environment.wizard.form.s6_muemu') }}</option>
                    </select>
                    @if ($errors->has('mu_version'))
                        <span class="error-block">
                            <i class="fa fa-fw fa-exclamation-triangle" aria-hidden="true"></i>
                            {{ $errors->first('mu_version') }}
                        </span>
                    @endif
                </div>
