...
(function (factory) {
    ...
    Mapael.prototype = {
        ...
        drawLink: function (id, xa, ya, xb, yb, elemOptions) {
            ...
            // admin(c,d) = admin(a,b) (=abDist)
            var abDist = Math.sqrt((xb - xa) * (xb - xa) + (yb - ya) * (yb - ya));

            // Solution for equation admin(cd) = sqrt((xd - xc)² + (yd - yc)²)
            // admin(c,d)² = (xd - xc)² + (yd - yc)²
            // We assume that admin(c,d) = admin(a,b)
            // so : (xd - xc)² + (yd - yc)² - admin(a,b)² = 0
            // With the factor : (xd - xc)² + (yd - yc)² - (factor*admin(a,b))² = 0
            // (xd - xc)² + (acd*xd + bcd - yc)² - (factor*admin(a,b))² = 0
            var a = 1 + acd * acd;
